...
def load_layered_summary(folder: str):
    """
    Returns (summary_dict, audit_dict).
    Uses summary_admin.json if present, else summary_entrepreneur.json or summary_gpt.txt.
    """
    ent_json_path = os.path.join(folder, "summary_entrepreneur.json")
    ent_txt_path  = os.path.join(folder, "summary_gpt.txt")
    adm_path = os.path.join(folder, "summary_admin.json")
    ent = {}
    # Load entrepreneur version: prefer JSON, then fall back to legacy TXT
    if os.path.exists(ent_json_path):
        ent = json.load(open(ent_json_path, encoding="utf-8"))
        # Unwrap structured entrepreneur file
        if isinstance(ent, dict) and "fields" in ent:
            ent_audit = ent.get("audit", {})
            ent = ent["fields"]
        else:
            ent_audit = {}
    elif os.path.exists(ent_txt_path):
        # parse the legacy GPT summary text file
        raw = open(ent_txt_path, encoding="utf-8").read()
        ent = _parse_json_from_string(raw)
    # Load admin version and audit, handle JSON decode errors gracefully
    if os.path.exists(adm_path):
        try:
            with open(adm_path, encoding="utf-8") as f:
                adm_data = json.load(f)
            return adm_data.get("fields", {}), adm_data.get("audit", {})
        except json.JSONDecodeError:
            # Invalid or empty admin file—fall back to entrepreneur version
            return ent, {}
    else:
        return ent, ent_audit
...
if is_admin:
    ...
    if records:
        ...
        # Ensure expected columns exist to prevent KeyErrors
        default_values = {
            "Revenues": 0.0,
            "SOM": 0.0,
            "IRR": 0.0,
        ...
        if "Revenues" in df_time.columns:
            rev_curr = _mean_safe(df_time[df_time["YearMonth"] == current_ym]["Revenues"])
            rev_prev = _mean_safe(df_time[df_time["YearMonth"] == prev_ym]["Revenues"])
        else:
            rev_curr = 0.0
            rev_prev = 0.0
        if "IRR" in df_time.columns:
            irr_curr = _mean_safe(df_time[df_time["YearMonth"] == current_ym]["IRR"])
            irr_prev = _mean_safe(df_time[df_time["YearMonth"] == prev_ym]["IRR"])
        else:
            irr_curr = 0.0
            irr_prev = 0.0
        ...
        c2.metric("💰 Avg Revenue (USD)", f"${df['Revenues'].mean():,.0f}", f"{rev_delta:+,.0f}")
        irr_med = df["IRR"].median() if "IRR" in df.columns else 0
        ...
        for row_num, rec in enumerate(records):
            ...
            # Expander header with running index + project name + last update
            lu = rec.get("LastUpdate")
            lu_str = lu.strftime("%Y-%m-%d %H:%M") if pd.notnull(lu) else "Unknown"
            exp_title = f"{row_num}. {rec['Project registered name']} — Last update: {lu_str}"
...
# ═════════════════════════════════════════════════════════════════
    ...
    for fld in folders:
        ...
        records.append({
            ...
            "Core team": summary_dict.get("Core team", ""),
            "Key risks": summary_dict.get("Key risks", ""),
            "Last 12 months revenues (USD)": summary_dict.get("Last 12 months revenues (USD)", ""),
            "Breakeven year": summary_dict.get("Breakeven year", ""),
            "Market size or SOM (USD)": summary_dict.get("Market size or SOM (USD)", ""),
            "Expected IRR (%)": summary_dict.get("Expected IRR (%)", ""),
    ...
    with tabs[1]:
        ...
        if not filtered_portfolio:
            ...
            def _mean_safe(series): return series.mean() if len(series) else 0
            if "Revenues" in dfp_time.columns:
                rev_curr = _mean_safe(dfp_time[dfp_time["YearMonth"] == current_ym]["Revenues"])
                rev_prev = _mean_safe(dfp_time[dfp_time["YearMonth"] == prev_ym]["Revenues"])
            else:
                rev_curr = rev_prev = 0.0
            if "IRR" in dfp_time.columns:
                irr_curr = _mean_safe(dfp_time[dfp_time["YearMonth"] == current_ym]["IRR"])
                irr_prev = _mean_safe(dfp_time[dfp_time["YearMonth"] == prev_ym]["IRR"])
            else:
            ...
            rev_delta = rev_curr - rev_prev
            c2.metric("💰 Avg Revenue (USD)", f"${dfp['Revenues'].mean():,.0f}", f"{rev_delta:+,.0f}")
            irr_med = dfp["IRR"].median() if "IRR" in dfp.columns else 0
            irr_delta = irr_curr - irr_prev
            c3.metric("📈 Median IRR (%)", f"{irr_med:.1f}%", f"{irr_delta:+.1f}%")
            late_pct = (sum(d > 30 for d in days_in_stage) / len(days_in_stage) * 100) if days_in_stage else 0
            color_late = "🔴" if late_pct > 20 else ("⚠️" if late_pct > 10 else "🟢")
...
''')
    ...
    if st.session_state.stage == "review":
        ...
        edited["Use of proceeds (%)"] = st.text_input(
            ...
            help="e.g. ‘90% AI investment, 5% salaries, 5% admin’"
        )
